...
\begin{algorithm}
	...
	\KwIn{
		\begin{itemize}
			...
			\item \textbf{Minimum path gain threshold} $P_{\text{th}}$: The threshold for acceptable signal strength, below which cells are considered low-power.
	...
	Set a minimum path gain threshold $P_{\text{th}}$ for acceptable signal quality.\\
	Identify low-power cells in $\mathbf{P}_{\text{TX}}(x, y)$ where the path gain is below the minimum path gain threshold $P_{\text{th}}$, denoted as $\mathcal{C}_{\text{low}}$:
	\begin{equation}
...
\begin{algorithm}
	...
	\ForEach{number of target points $N \in \mathcal{N}$}{
		...
		\ForEach{RIS width $W_{\text{RIS}} \in \mathcal{W}$}{
			...
			Calculate the performance metric $\mathcal{M}(\mathbf{r}_{\text{RIS}}, N, W_{\text{RIS}})$ for all parameter combinations as the average path gain of low-power cells after placing the RIS at $\mathbf{r}_{\text{RIS}}$:
			\begin{equation}
...
\begin{figure}
	...
	\caption{Binary poor coverage maps for different number of target configurations for the minimum path gain threshold of $-100$ dB}
	\label{binary_maps}
\end{figure}
...
\begin{figure}
	...
	c) $N = 4$ \hspace{80pt} d) $N = 5$
	\caption{Binary poor coverage maps for different number of target configurations for the minimum path gain threshold of $-110$ dB}
